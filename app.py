...
def make_prediction(input_video):
    ...
    if not valid_input:
        return {
            "error": "The input video should have only one person with a clearly visible face"
        }
    ...
    while True:
        ...
        if frame_count == 32:
            ...
            prediction = (
                saved_model.predict(input_data) if saved_model else [0]
            )  # Dummy prediction
...
def split_and_pad_audio(audio_file, segment_duration=10, sample_rate=16000):
    ...
    if remainder > 0:
        ...
        padded_segment = np.pad(
            last_segment, (0, segment_samples - len(last_segment)), mode="constant"
        )
...
def final_decision(input_video):
    ...
    if len(replicated_voice_predictions) > len(facial_predictions):
        replicated_voice_predictions = replicated_voice_predictions[
            : len(facial_predictions)
        ]
    # If replicated voice predictions are shorter than facial predictions, pad them
        ...
        replicated_voice_predictions = np.pad(
            replicated_voice_predictions,
            (0, len(facial_predictions) - len(replicated_voice_predictions)),
            constant_values=1,
        )
